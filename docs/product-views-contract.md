...
As coleções vêm de `pack_collections`, filtradas por `is_visible` e ordenadas por `sort_order`. Cada coleção entrega `cover_url` e `cover_ratio`; a Members deve preservar a proporção cadastrada em sua própria composição visual. Capas e etiquetas podem ser usadas na navegação e nos filtros. Os conteúdos vêm de `pack_items`, filtrados por `status = published` e também ordenados por `sort_order`. Cada item também entrega `cover_url` e `cover_ratio`, cujo valor é `1:1`, `16:9`, `4:3`, `3:4` ou `9:16`; a Members deve preservar essa proporção sem herdar a lista editorial administrativa do Hub e sem forçar um recorte único. Coleções e itens antigos usam `16:9` como padrão. Itens vinculados a uma coleção oculta não podem ser exibidos; itens sem coleção continuam válidos. A leitura por aluno é permitida apenas com matrícula ativa e não expirada.
...
5. Ordenar coleções, itens e vídeos por `sort_order`, respeitar `cover_ratio` na mídia de cada item e fornecer estados vazios e de conteúdo indisponível.
