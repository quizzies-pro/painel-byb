...
As coleções vêm de `pack_collections`, filtradas por `is_visible` e ordenadas por `sort_order`. Capas e etiquetas podem ser usadas na navegação e nos filtros. Os conteúdos vêm de `pack_items`, filtrados por `status = published` e também ordenados por `sort_order`. Cada item entrega `cover_url` e `cover_ratio`, cujo valor é `1:1`, `16:9`, `4:3`, `3:4` ou `9:16`; a Members deve preservar essa proporção em sua própria composição visual, sem herdar a lista editorial administrativa do Hub e sem forçar um recorte único. Itens antigos usam `16:9` como padrão. Itens vinculados a uma coleção oculta não podem ser exibidos; itens sem coleção continuam válidos. A leitura por aluno é permitida apenas com matrícula ativa e não expirada.
