...
const EMPTY_COLLECTION = { title: "", description: "", cover_url: "", cover_ratio: "16:9" as PackItemCoverRatio, tags: [] as string[], is_visible: true };
...
export default function PackContentPage() {
  ...
  const openCollection = (collection?: Collection) => {
    ...
    setCollectionForm(collection ? { title: collection.title, description: collection.description ?? "", cover_url: collection.cover_url ?? "", cover_ratio: collection.cover_ratio, tags: collection.tags ?? [], is_visible: collection.is_visible } : EMPTY_COLLECTION);
    setCollectionOpen(true);
  };

  const saveCollection = async () => {
    if (!courseId || !collectionForm.title.trim()) return toast.error("Informe o nome da coleção");
    const values = { title: collectionForm.title.trim(), description: collectionForm.description.trim() || null, cover_url: collectionForm.cover_url.trim() || null, cover_ratio: collectionForm.cover_ratio, tags: collectionForm.tags, is_visible: collectionForm.is_visible };
  ...
  const deleteCollection = async (collection: Collection) => {
    if (!await confirmAction({ description: `Excluir a coleção “${collection.title}”? Os itens ficarão sem coleção.` })) return;
  ...
  const deleteItem = async (item: PackItem) => {
    if (!await confirmAction({ description: `Excluir “${item.title}”?` })) return;
  ...
  const deleteVideo = async (video: PackVideo) => {
    if (!await confirmAction({ description: `Excluir o vídeo “${video.title}”?` })) return;
    ...
    <div className="space-y-7">
      ...
      {collections.length === 0 && items.length === 0 ? (
        ...
        {collections.map((collection, index) => <section key={collection.id} className="space-y-4">
          <div className="flex items-center justify-between border-b border-border pb-3"><div className="flex items-center gap-3">{collection.cover_url && <div className="flex h-16 w-20 shrink-0 items-center justify-center rounded bg-muted/30"><img src={collection.cover_url} alt="" className={`max-h-14 max-w-[4.5rem] rounded object-contain ${COVER_RATIO_PREVIEW_CLASSES[collection.cover_ratio]}`} /></div>}<div><div className="flex items-center gap-2"><h2 className="font-medium">{collection.title}</h2><Badge variant="outline" className="text-[10px] font-normal">{collection.cover_ratio}</Badge>{!collection.is_visible && <Badge variant="secondary" className="text-[10px]">Oculta</Badge>}</div>{collection.description && <p className="mt-1 text-xs text-muted-foreground">{collection.description}</p>}{collection.tags.length > 0 && <div className="mt-2 flex flex-wrap gap-1">{collection.tags.map((tag) => <Badge key={tag} variant="outline" className="text-[10px] font-normal">{tag}</Badge>)}</div>}</div></div><div className="flex gap-1"><Button size="icon" variant="ghost" disabled={index === 0} onClick={() => moveRecord("collection", collections, index, -1)} title="Mover coleção para cima"><ArrowUp className="h-4 w-4" /></Button><Button size="icon" variant="ghost" disabled={index === collections.length - 1} onClick={() => moveRecord("collection", collections, index, 1)} title="Mover coleção para baixo"><ArrowDown className="h-4 w-4" /></Button><Button size="icon" variant="ghost" onClick={() => openCollection(collection)}><Pencil className="h-4 w-4" /></Button><Button size="icon" variant="ghost" className="text-muted-foreground hover:text-destructive" onClick={() => deleteCollection(collection)}><Trash2 className="h-4 w-4" /></Button></div></div>
      ...
      <Dialog open={driveOpen} onOpenChange={setDriveOpen}><DialogContent size="explorer"><DialogHeader><DialogTitle>Selecionar arquivos do Google Drive</DialogTitle></DialogHeader><DialogBody className="space-y-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><Select value={driveCollectionId} onValueChange={setDriveCollectionId}><SelectTrigger className="w-full sm:w-64"><SelectValue placeholder="Coleção de destino" /></SelectTrigger><SelectContent><SelectItem value="none">Sem coleção</SelectItem>{collections.map((collection) => <SelectItem key={collection.id} value={collection.id}>{collection.title}</SelectItem>)}</SelectContent></Select>{driveFolderId !== product.drive_root_folder_id && <Button variant="outline" size="sm" onClick={() => loadDriveFolder(product.drive_root_folder_id ?? "")}>Voltar à pasta principal</Button>}</div><div className="max-h-[55vh] divide-y divide-border overflow-y-auto rounded-lg border border-border">{driveFiles.length === 0 ? <p className="p-8 text-center text-sm text-muted-foreground">Nenhum arquivo encontrado nesta pasta</p> : driveFiles.map((file) => {
