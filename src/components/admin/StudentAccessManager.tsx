...
export default function StudentAccessManager({ studentId, initialProductId, onChanged }: StudentAccessManagerProps) {
  ...
  const filteredProducts = useMemo(() => products.filter((product) => {
    ...
    return (!term || product.title.toLowerCase().includes(term))
      ...
      && (statusFilter === "all" || (statusFilter === "available" ? !enrollment : enrollment?.status === statusFilter));
  }), [enrollmentByProduct, products, search, statusFilter, typeFilter]);
  ...
  return <section className="student-workspace overflow-hidden rounded-lg border border-border bg-card">
    <div className="border-b border-border p-4 sm:p-5"><div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
      <div><h2 className="workspace-title text-lg font-semibold">Acessos e produtos</h2><p className="mt-1 text-sm text-muted-foreground">{enrollments.filter((item) => item.status === "active").length} ativos de {products.length} produtos</p></div>
      <div className="flex flex-col gap-2 sm:flex-row"><div className="relative min-w-0 sm:w-72"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar produto..." className="pl-9" /></div>
        <Select value={statusFilter} onValueChange={setStatusFilter}><SelectTrigger className="sm:w-40"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Todos os acessos</SelectItem><SelectItem value="available">Disponíveis</SelectItem><SelectItem value="active">Ativos</SelectItem><SelectItem value="expired">Expirados</SelectItem><SelectItem value="canceled">Cancelados</SelectItem><SelectItem value="blocked">Bloqueados</SelectItem></SelectContent></Select>
    ...
    {filteredProducts.length === 0 ? <div className="p-10 text-center text-sm text-muted-foreground">Nenhum produto encontrado</div> : filteredProducts.map((product) => {
      ...
      return <div key={product.id} className="border-b border-border last:border-b-0">
        ...
        </div><div className="mt-6 flex justify-end gap-2 border-t border-border pt-4"><Button type="button" variant="outline" onClick={() => setExpandedProductId(null)}>Cancelar</Button><Button type="button" onClick={() => saveAccess(product)} disabled={saving}>{saving ? "Salvando..." : enrollment ? "Salvar alterações" : "Liberar acesso"}</Button></div></div>}
      </div>;
    })}
  </section>;
}
